...
customHeatmap <- function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
    ...
    lmat = NULL, lhei = NULL, lwid = NULL, extrafun = NULL, ColLabColors = 'black', RowLabColors = 'black', titleColor = 'black', ...) 
{
    ...
    if (is.null(srtCol)) 
        ...
        #    padj = adjCol[2])
        mtext(side = 1, text = labCol, at = 1:nc, las = 1, line = 0.5, col = ColLabColors, cex = cexCol)
        ...
        if (is.numeric(srtCol)) {
            ...
            text(x = xpos, y = par("usr")[3] - (1 + offsetCol) * 
                ...
                cex = cexCol + 2, srt = srtCol, col = ColLabColors)
            par(xpd = xpd.orig)
        }
    ...
    if (is.null(srtRow)) {
        ...
        mtext(side = 4, text = labRow, at = iy, las = 2, line = 0.5, col = RowLabColors, cex = cexRow)
    ...
    if (!is.null(main)) 
        title(main, cex.main = 1.5 * op[["cex.main"]], col.main = titleColor, font.main = 1)
    if (key) {
        mar <- c(5, 4, 2, 1)
